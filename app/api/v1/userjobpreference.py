...
from app.services.userjobpreference import (
    create_user_job_preference as create_user_job_preference_service,
)
...
def create_user_job_preference(
    ...
    success, result = create_user_job_preference_service(
        session, user_id, payload.keyword, payload.is_excluded
    )
    if not success:
